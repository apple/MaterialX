...
int GetStrideOfMetalType(MTLDataType type)
{
    switch(type)
    {
        ...
        case MTLDataTypeInt:    return 1 * 4;
        default:                return 0;
    }
    return 0;
}
...
void MslProgram::bindAttribute(id<MTLRenderCommandEncoder> renderCmdEncoder, const MslProgram::InputMap& inputs, MeshPtr mesh)
{
    ...
    for (const auto& input : inputs)
    {
        ...
        if (_attributeBufferIds.find(input.first) == _attributeBufferIds.end())
        {
            ...
            {
                int nElements = (int)attributeData.size()/stride;
    ...
    for (MTLArgument *arg in _psoReflection.fragmentArguments)
    {
        if (arg.type == MTLArgumentTypeTexture)
        {
            ...
            if(lightHandler)
            {
                ...
                for (const auto& env : envLights)
                {
                    ...
                    if(loc != std::string::npos && env.second)
                    {
                        ...
                        bindTexture(renderCmdEncoder, (int)arg.index, env.second, imageHandler);
                        found = true;
                    }
            ...
            if(!found)
            {
                ...
                {
                    bindTexture(renderCmdEncoder, (int)arg.index, image, imageHandler);
                    found = true;
                }
            }
            ...
            if(!found)
            {
                ...
                if(uniform != _uniformList.end())
                {
                    ...
                    bindTexture(renderCmdEncoder, (int)arg.index, fileName, samplingProperties, imageHandler);
                }
            }
        }
    }
    ...
    if (_shader)
    {
        ...
        for (const auto& uniformMap : ps.getUniformBlocks())
        {
            ...
            for (size_t i = 0; i < uniforms.size(); ++i)
            {
                ...
                int resourceType = (int)mapTypeToMetalType(v->getType());
                ...
                if (resourceType == Input::INVALID_METAL_TYPE)
                {
                    continue;
                }
...
MTLDataType MslProgram::mapTypeToMetalType(const TypeDesc* type)
{
    ...
    return (MTLDataType)MslProgram::Input::INVALID_METAL_TYPE;
}
